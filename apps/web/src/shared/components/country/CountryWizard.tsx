...
export default function CountryWizard({ onComplete, onCancel }: Props) {
  ...
  const [identity, setIdentity] = useState({
    ...
    timezone: "Africa/Johannesburg", flagEmoji: "",
  });
            ...
            <>
              ...
              <Row2>
                ...
                <Field label="Flag emoji">
                  <input value={identity.flagEmoji} onChange={e => setIdentity(p => ({ ...p, flagEmoji: e.target.value }))} placeholder="🇿🇦" style={{ ...inputSt, fontSize: 20 }} />
                </Field>
              </Row2>
            </>
          )}
            ...
            <>
              <div style={{ display: "flex", gap: 8, padding: "12px 16px", background: "rgba(30,140,110,0.08)", border: "1px solid rgba(30,140,110,0.2)", borderRadius: 10, alignItems: "flex-start" }}>
                ...
                <p style={{ margin: 0, color: "#1e8c6e", fontSize: 13, lineHeight: 1.5 }}>
                  All steps complete. <strong>{identity.flagEmoji} {identity.name}</strong> is ready to publish. Choose a status below — you can change this at any time.
                </p>
              </div>
              ...
              <div style={{ background: "var(--color-bg-subtle)", borderRadius: 12, border: `1px solid ${BORD}`, overflow: "hidden" }}>
                {[
                  { label: "Country",    value: `${identity.flagEmoji} ${identity.name} (${identity.code})` },
