...
export default function CountryDetailPage() {
  ...
  const startEdit = (section: string) => {
    ...
    if (section === "identity")   setDraftIdentity({ name: country.name, code: country.code, currency: country.currency, currencySymbol: country.currencySymbol, timezone: country.timezone });
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <button onClick={() => router.push("/admin/countries")} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--color-text-muted)", background: "none", border: "none", cursor: "pointer", marginBottom: 10, padding: 0 }}>
            <ArrowLeft size={14} /> Back to Chapter Management
          </button>
