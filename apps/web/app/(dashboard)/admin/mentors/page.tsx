...
export default function MentorsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient.get(endpoints.admin.mentors, { params });
      setUsers(res.data.data.mentors ?? []);
  ...
  const executeAction = async (action: UserAction, user: UserRow) => {
    try {
      if (action === "activate" || action === "suspend") {
        ...
        showToast(`Mentor ${action === "activate" ? "activated" : "suspended"}`);
        ...
        showToast("Mentor permanently deleted");
      }
      load();
    } catch (e: any) {
  ...
  const handleInvite = async (firstName: string, lastName: string, email: string) => {
    await apiClient.post(endpoints.admin.mentorInvite, { firstName, lastName, email });
  ...
  return (
    <div style={{ minHeight: "100vh", padding: 32, background: "var(--color-content-bg)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 800, color: "var(--color-text-primary)" }}>Mentors</h1>
          <p style={{ margin: 0, fontSize: 13, color: "var(--color-text-muted)" }}>
            Invite and manage mentors. They can be assigned to cohorts.
          </p>
        </div>
        <button
      ...
      <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORD}`, overflow: "hidden" }}>
        ...
        <UserTable
          ...
          extraColumns={[
            {
              header: "Cohorts",
              render: u => `${u._count?.cohortMentorships ?? 0} assigned`,
            },
          ]}
        />
      </div>
      ...
      {showInvite && (
        <InviteModal role="Mentor" onClose={() => setShowInvite(false)} onInvite={handleInvite} />
      )}

      {confirm?.action === "hard_delete" && (
        <ConfirmModal
          title="Hard Delete Mentor"
