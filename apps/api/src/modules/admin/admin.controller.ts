...
export const adminDashboard = catchAsync(async (_req: Request, res: Response) => {
  const [totalUsers, totalManagers, pendingUsers, activeUsers, recentSignups, recentActivity] =
    await Promise.all([
      prisma.user.count({ where: { role: "MEMBER",    accountStatus: { not: "DELETED" } } }),
      prisma.user.count({ where: { role: "MANAGER", accountStatus: { not: "DELETED" } } }),
      prisma.user.count({ where: { role: "MEMBER",    accountStatus: "PENDING" } }),
...
export const inviteMentor = catchAsync(async (req: Request, res: Response) => {
  ...
  await prisma.auditLog.create({
    data: { userId: req.user!.userId, action: "MENTOR_INVITED", meta: { email } },
  });
  req.auditLogged = true;
...
export const listMentors = catchAsync(async (req: Request, res: Response) => {
  ...
  const mentors = await prisma.user.findMany({
    ...
    select: {
      id: true, email: true, firstName: true, lastName: true,
      displayName: true, avatarUrl: true, accountStatus: true,
  ...
  return res.status(HttpStatus.OK).json({ status: "success", data: { mentors } });
});
...
export const resendUserInvite = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;

  const user = await prisma.user.findUnique({ where: { id } });
  if (!user) throw new AppError("User not found", HttpStatus.NOT_FOUND);
  if (user.accountStatus !== "PENDING")
    throw new AppError("User has already accepted their invitation", HttpStatus.BAD_REQUEST);
